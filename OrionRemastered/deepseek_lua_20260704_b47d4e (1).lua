...
-- Örnek Kullanım (Bu kod, kütüphane dosyasının sonuna eklenebilir veya ayrı bir scriptte çalıştırılabilir)
    ...
    local Window = OrionLib:MakeWindow({
        ...
        SaveConfig = true,
        IntroEnabled = true,
        IntroText = "Orion Recontinued",
        Gradient = {
            Color3.fromRGB(20, 20, 30),
            Color3.fromRGB(40, 40, 60),
            Color3.fromRGB(20, 20, 30)
        }, -- Gradient renkleri (ColorSequence)
        ShowIcon = true,
    ...
    -- Tab1 içinde elementler
    Tab1:AddLabel("Hoş geldiniz!")
    ...
    local multi = Tab1:AddMultiDropdown({
        Name = "Multi Dropdown",
        Options = {"A", "B", "C", "D"},
        Default = {"A", "C"},
        Callback = function(v)
    ...
    local bind = Tab1:AddBind({
        Name = "Bind",
        Default = Enum.KeyCode.R,
        Hold = false,
        Callback = function()
            print("Bind tetiklendi!")
        end,
        Flag = "bind1",
        Save = true
    })
   ...
   --[[local colorpicker = Tab1:AddColorpicker({
    ...
    -- Tab2 içinde elementler
    Tab2:AddLabel("Ayarlar bölümü")
    Tab2:AddButton({
        Name = "Seçenek 1",
        Callback = function()
            print("Seçenek 1")
    ...
    -- Goto örneği: Tab1'deki 4. elemente git (dropdown)
    wait(2)
    Window:GotoElement("Ana", 4)

    -- Seçili tab'ı değiştir
    wait(1)
    Window:SelectTab("Ayarlar")

    -- Pencereyi kapatmak için (popup ile)
    wait(2)
    Window:Close() -- Bu popup açacaktır.
